...
from _CoreGraphics_inlines import _inline_list_

sys.modules['Quartz.CoreGraphics'] = mod = objc.ObjCLazyModule('Quartz.CoreGraphics',
    "com.apple.CoreGraphics",
#                                                               objc.__path__[0]+"/Frameworks/CoreGraphics.framework",
                                                               None,
...
def _load(mod):
    ...
    import _CoreGraphics_callbacks as m
    for nm in dir(m):
...
#        setattr(mod, nm, getattr(m, nm))
    ...
    import _sortandmap as m
    for nm in dir(m):
        if nm.startswith('_'): continue
        setattr(mod, nm, getattr(m, nm))

    import _coregraphics as m
    for nm in dir(m):
        if nm.startswith('_'): continue
        setattr(mod, nm, getattr(m, nm))
